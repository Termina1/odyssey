...
try {
	manifest = parseJsonText(stdin, TakeManifest, "completion event");
} catch (error) {
	fail(`take guard: ${error instanceof Error ? error.message : String(error)}`);
}
const cwd = resolve(process.cwd());
const artifactPath = resolve(cwd, manifest.artifactPath);
if (artifactPath !== cwd && !artifactPath.startsWith(`${cwd}${sep}`))
	fail("take guard: artifactPath escapes run directory");
let research!: ReturnType<typeof DeepResearch.parse>;
try {
	research = await parseJsonFile(artifactPath, DeepResearch);
} catch (error) {
