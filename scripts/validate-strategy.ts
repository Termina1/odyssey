...
const invalid = (reason: string): never => { console.log(JSON.stringify({ type: "STRATEGY_INVALID", output: { reason, instructions: [reason] } })); process.exit(0); };
let strategy!: ReturnType<typeof NarrativeStrategy.parse>;
let evidence!: ReturnType<typeof EvidenceIndex.parse>;
try {
  [strategy, evidence] = await Promise.all([parseJsonFile(process.env.STRATEGY_FILE ?? "", NarrativeStrategy), parseJsonFile(process.env.EVIDENCE_FILE ?? "", EvidenceIndex)]);
} catch (error) { invalid(`Cannot read authoritative strategy inputs: ${error instanceof Error ? error.message : String(error)}`); }
...
for (const section of strategy.sections) {
  ...
  for (const evidenceId of section.evidenceIds) if (!evidenceIds.has(evidenceId)) invalid(`strategy section ${section.id} references unknown evidence ${evidenceId}`);
}
