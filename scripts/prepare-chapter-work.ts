...
import { ExperiencePlan, ReportPlan, SectionWorkItems, type SectionWorkItem } from "../contracts/index.js";
...
for (let index = 0; index < plan.sections.length; index += 1) {
  ...
  const beats = section.beatIds.map((id) => beatsById.get(id)).filter((beat): beat is ReportPlan["beats"][number] => Boolean(beat));
  items[section.id] = {
    sectionId: section.id, index, section, beats, experience: experienceSection,
