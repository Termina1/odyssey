...
const requireDataset = (
	value: ReturnType<typeof VisualInput.parse>["dataset"],
): NonNullable<ReturnType<typeof VisualInput.parse>["dataset"]> => {
	if (value) return value;
	invalid("usable dataset request has no plot-ready rows");
	throw new Error("unreachable");
};
const requireImage = (
	value: ReturnType<typeof VisualInput.parse>["image"],
): NonNullable<ReturnType<typeof VisualInput.parse>["image"]> => {
	if (value) return value;
	invalid("usable image request has no localPath");
	throw new Error("unreachable");
};
...
try {
	...
	[input, evidence] = await Promise.all([
		parseJsonFile(process.env.INPUT_FILE ?? "", VisualInput),
		parseJsonFile(process.env.EVIDENCE_FILE ?? "", EvidenceIndex),
	]);
} catch (error) {
	invalid(`Cannot read visual input: ${error instanceof Error ? error.message : String(error)}`);
}
...
const allowedSourceIds = new Set(
	evidence.evidence.filter((entry) => requestedEvidence.has(entry.id)).flatMap((entry) => entry.sourceIds),
);
for (const sourceId of input.sourceIds) {
	if (sourceId.startsWith("e_"))
		invalid(
			`sourceIds contains evidence ID ${sourceId}; use only source-record IDs with the s_ prefix, and put evidence IDs in dataset.provenance[].evidenceId`,
		);
...
for (const entry of input.dataset?.provenance ?? [])
	if (entry.evidenceId !== undefined && !requestedEvidence.has(entry.evidenceId))
		invalid(`dataset provenance references unrelated evidence ${entry.evidenceId}`);
...
for (const url of input.sourceUrls) {
	try {
		...
		invalid(`invalid source URL ${url}`);
	}
}
if (input.status === "usable" && request.kind === "dataset") {
	...
	for (const row of usableDataset.rows)
		for (const key of Object.keys(row)) if (!fields.has(key)) invalid(`dataset row uses undeclared field ${key}`);
}
if (input.status === "usable" && ["image", "screenshot"].includes(request.kind)) {
	...
	try {
		await access(resolve(process.cwd(), usableImage.localPath));
	} catch {
		invalid(`image file is missing: ${usableImage.localPath}`);
	}
}
