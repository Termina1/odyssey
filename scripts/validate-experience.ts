...
try {
	[plan, experience] = await Promise.all([
		parseJsonFile(process.env.PLAN_FILE ?? "", ReportPlan),
		parseJsonFile(process.env.EXPERIENCE_FILE ?? "", ExperiencePlan),
	]);
} catch (error) {
	finish(
		"EXPERIENCE_INVALID",
		`Cannot read authoritative inputs: ${error instanceof Error ? error.message : String(error)}`,
	);
}
...
for (const sectionId of sectionIds)
	if (!experience.sections[sectionId]) invalid(`Missing experience section ${sectionId}`);
for (const [sectionId, section] of Object.entries(experience.sections)) {
	if (!sectionIds.has(sectionId) || section.sectionId !== sectionId)
		invalid(`Unknown or mismatched experience section ${sectionId}`);
	...
	for (const beatId of Object.keys(section.beats))
		if (!planBeatIds.has(beatId)) invalid(`Section ${sectionId} references unknown beat ${beatId}`);
