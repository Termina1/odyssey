...
for (let index = 0; index < draft.beats.length; index += 1) {
	...
	const packet = {
		...
		sources: sourceIds
			...
			.filter((source): source is EvidenceIndexType["sources"][number] => Boolean(source)),
	};
	await writeFile(packetPath, `${JSON.stringify(packet, null, 2)}\n`);
	items[key] = workItem;
}
const output = BeatItems.parse({
	items,
	count: Object.keys(items).length,
	artifactPath: process.env.OUTPUT_PATH ?? "artifacts/plan/beat-items.json",
});
