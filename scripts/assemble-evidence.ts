...
import { z } from "zod";
import { DeepResearch, EvidenceIndex, type DeepResearch as DeepResearchType, type EvidenceIndex as EvidenceIndexType } from "../contracts/index.js";
...
for (const take of takes) {
  ...
  for (const finding of take.findings) {
    const sourceIds = [...new Set(finding.sourceIds.map((id) => localSources.get(id)).filter((id): id is string => Boolean(id)))].sort();
    const claim = finding.claim.trim();
    const id = `e_${hash(`${claim.toLowerCase()}|${sourceIds.join(",")}`)}`;
    const existing = evidenceById.get(id);
    const next: EvidenceIndexType["evidence"][number] = { id, claim, sourceIds, confidence: finding.confidence, caveat: finding.caveat ?? "", tags: [...new Set(finding.tags)].sort(), takeIds: [take.takeId] };
    if (existing) {
