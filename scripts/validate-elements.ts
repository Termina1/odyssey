import { ElementPackage, SectionWorkItem, VisualCatalog, type Dataset } from "../contracts/index.js";
...
try {
  ...
  [elements, catalog] = await Promise.all([parseJsonFile(process.env.ELEMENTS_FILE ?? "", ElementPackage), parseJsonFile(process.env.VISUAL_CATALOG_FILE ?? "", VisualCatalog)]);
...
for (const block of elements.blocks) {
  ...
  if (block.type === "chart" || block.type === "metric-strip" || block.type === "table") {
    ...
    if (!visual || visual.status !== "usable" || !visual.dataset) { invalid(`block ${block.id} has no usable dataset`); continue; }
    const dataset: Dataset = visual.dataset;
    const fields = new Set(dataset.fields.map((field) => field.key));
    if (block.type === "chart") {
      try { validateChartContract(block, dataset); }
      catch (error) { invalid(`block ${block.id}: ${error instanceof Error ? error.message : String(error)}`); }
    }
    if (block.type === "table") for (const field of block.columns) if (!fields.has(field)) invalid(`block ${block.id} table references unknown dataset field ${field}`);
    if (block.type === "metric-strip") for (const metric of block.metrics) {
      ...
      const matches = dataset.rows.filter((row) => Object.entries(metric.where).every(([field, value]) => row[field] === value));
