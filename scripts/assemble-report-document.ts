...
for (const section of plan.sections) {
	...
	if (
		...
		elementPackage.sectionId !== section.id ||
		visualCatalog.sectionId !== section.id
	)
		throw new Error(`Chapter identity mismatch for ${section.id}`);
	chapters.push(chapter);
	elements.push(elementPackage);
	visualInputs.push(...visualCatalog.inputs);
}
const output = ReportDocument.parse({
	version: "1",
...
console.log(
	JSON.stringify({
		type: "DOCUMENT_READY",
		output: {
			artifactPath: process.env.OUTPUT_PATH,
			sections: chapters.length,
			blocks: elements.reduce((sum, entry) => sum + entry.blocks.length, 0),
		},
	}),
);
