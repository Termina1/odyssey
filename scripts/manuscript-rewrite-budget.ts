...
const output = { reason: feedback.reason, chapters: feedback.chapters, ...(feedback.engineIssues ? { engineIssues: feedback.engineIssues } : {}) };
if (Number.isFinite(visit) && visit <= MAX_TARGETED_REWRITES) console.log(JSON.stringify({ type: "RETRY", output }));
else console.log(JSON.stringify({ type: "CONTINUE_WITH_WARNINGS", output: { ...output, reason: `Manuscript rewrite budget exhausted after ${MAX_TARGETED_REWRITES} targeted passes. ${output.reason}`.trim() } }));
