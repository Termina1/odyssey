...
export function validateChartContract(block, dataset) {
  ...
  const enc = block.encoding;
  if (!enc || typeof enc !== "object" || Array.isArray(enc)) throw new Error(`Chart ${block.id ?? "(unnamed)"} encoding must be an object`);
  for (const key of Object.keys(enc)) if (!ALLOWED_ENCODINGS.has(key)) throw new Error(`Chart ${block.id ?? "(unnamed)"} declares unsupported encoding channel ${key}`);
  for (const key of Object.keys(enc)) if (enc[key] !== undefined && (typeof enc[key] !== "string" || !fields.has(enc[key]))) throw new Error(`Chart ${block.id ?? "(unnamed)"} encoding.${key} references unknown field ${String(enc[key])}`);
  const required = {
    "line": ["x", "y"], "area": ["x", "y"], "bar": ["x", "y"], "stacked-bar": ["x", "y"], "grouped-bar": ["x", "y"], "100%-stacked-bar": ["x", "y"],
    "scatter": ["x", "y"], "bubble": ["x", "y"], "heatmap": ["x", "y", "value"], "treemap": [], "sunburst": [], "sankey": ["source", "target", "value"],
  }[block.variant];
  for (const key of required) if (typeof enc[key] !== "string") throw new Error(`Chart ${block.id ?? "(unnamed)"} requires encoding.${key}`);
  if (["treemap", "sunburst"].includes(block.variant)) {
    if (typeof (enc.value ?? enc.y) !== "string") throw new Error(`Chart ${block.id ?? "(unnamed)"} requires encoding.value or encoding.y`);
    if (typeof (enc.name ?? enc.x) !== "string") throw new Error(`Chart ${block.id ?? "(unnamed)"} requires encoding.name or encoding.x`);
  }
  const numericKeys = new Set(["value", "size"]);
  if (["line", "area", "bar", "stacked-bar", "grouped-bar", "100%-stacked-bar", "scatter", "bubble"].includes(block.variant)) numericKeys.add("y");
  if (["scatter", "bubble"].includes(block.variant)) numericKeys.add("x");
  for (const key of numericKeys) {
    const field = enc[key];
    if (field && fields.get(field)?.type !== "number") throw new Error(`Chart ${block.id ?? "(unnamed)"} encoding.${key} must reference a number field`);
  }
  ...
  if (block.annotations !== undefined && !Array.isArray(block.annotations)) throw new Error(`Chart ${block.id ?? "(unnamed)"} annotations must be an array`);
...
function chartValue(value, field, normalized = false) {
  ...
  if (normalized || field?.unit === "%" || /%|percent|дол/i.test(field?.unit ?? "") || /%|percent|дол/i.test(field?.label ?? "")) return `${rounded}%`;
...
function transformCartesian(block, dataset, contract) {
  ...
  const pointColorKey = ["line", "area"].includes(block.variant) && !enc.series && enc.color && rows.length === categories.length && uniqueValues(rows, enc.color).length === rows.length ? enc.color : undefined;
  ...
  const series = seriesValues.map((seriesName, seriesIndex) => {
    const data = categories.map((category) => {
      ...
      if (pointColorKey) {
        ...
        datum.itemStyle = { ...datum.itemStyle, color: PALETTE[pointColorValues.indexOf(canonical(cell.row[pointColorKey])) % PALETTE.length] };
      }
      return datum;
    });
    const output = {
      ...
      label: { show: true, position: block.variant.includes("bar") ? (normalized ? "inside" : "top") : "top", color: "#17323a", fontSize: 12 },
    };
  ...
  options.xAxis = { type: "category", data: categories, name: xField?.label ?? xKey, nameLocation: "middle", nameGap: 40, axisLabel: { color: "#42565b", fontSize: 12, hideOverlap: true, interval: categories.length > 12 ? 1 : 0 } };
  options.yAxis = { type: "value", name: normalized ? "Доля (%)" : fieldTitle(yField), nameLocation: "middle", nameGap: 52, min: normalized ? 0 : undefined, max: normalized ? 100 : undefined, axisLabel: { color: "#42565b", fontSize: 12, formatter: normalized ? "{value}%" : undefined }, splitLine: { lineStyle: { color: "#d6dfdf", type: "dashed" } } };
  options.series = series;
  if (hasForecast && categories.length) {
    const forecastCategories = categories.filter((category) => rows.some((row) => canonical(row[xKey]) === category && rowIsForecast(row, contract.forecastKey, pointColorKey ?? seriesKey)));
    const first = forecastCategories[0];
    const last = forecastCategories.at(-1);
    options.series[0].markArea = { silent: true, itemStyle: { color: "rgba(228,163,75,.12)" }, data: [[{ xAxis: first }, { xAxis: last }]] };
  }
  const annotation = annotationFor(options, block.annotations);
  if (annotation) options.series[0].markPoint = annotation;
  const tableRows = categories.map((category) => ({ category, values: seriesValues.map((seriesName) => {
    const cell = cellFor(category, seriesName);
    return { series: pointColorKey && cell ? canonical(cell.row[pointColorKey]) : seriesName, value: cell?.value ?? null, missing: !cell, forecast: rowIsForecast(cell?.row, contract.forecastKey, pointColorKey ?? seriesKey) };
  }) }));
  const noteKey = dataset.fields.find((field) => /annotation|note|comment/i.test(field.key))?.key;
  const excludedAnnotations = annotationOnlyRows.map((row) => ({ category: canonical(row[xKey]), value: numeric(row[yKey], `Chart ${block.id ?? "(unnamed)"} ${yKey}`), note: noteKey ? canonical(row[noteKey]) : "Excluded from the plotted series because it is not comparable." }));
...
function transformScatter(block, dataset, contract) {
  ...
  options.yAxis = { type: "value", name: fieldTitle(yField), nameLocation: "middle", nameGap: 50, axisLabel: { color: "#42565b" }, splitLine: { lineStyle: { color: "#d6dfdf", type: "dashed" } } };
  if (block.interaction.zoom) options.dataZoom = [{ type: "inside" }, { type: "slider", bottom: 12 }];
  options.series = series;
  const rows = dataset.rows.map((row) => ({ category: enc.name ? canonical(row[enc.name]) : "Observation", values: [{ series: seriesKey ? canonical(row[seriesKey]) : "Series", value: [numeric(row[enc.x], enc.x), numeric(row[enc.y], enc.y)] }] }));
...
function transformHeatmap(block, dataset, contract) {
  ...
  for (const row of dataset.rows) { const key = `${canonical(row[enc.x])}\u0000${canonical(row[enc.y])}`; if (byCell.has(key)) throw new Error(`Chart ${block.id ?? "(unnamed)"} has duplicate heatmap cell ${key}`); byCell.set(key, numeric(row[enc.value], enc.value)); }
  const values = [...byCell.values()]; const min = Math.min(...values); const max = Math.max(...values); const options = baseOption({ interaction: block.interaction, legend: false, gridBottom: 56, ariaDescription: `${block.title ?? "Heatmap"}. ${xs.length} by ${ys.length} matrix.` });
...
function transformHierarchy(block, dataset, contract) {
  ...
  for (const row of dataset.rows) totals.set(canonical(row[nameKey]), (totals.get(canonical(row[nameKey])) ?? 0) + numeric(row[valueKey], valueKey));
...
function transformSankey(block, dataset, contract) {
  ...
  for (const row of dataset.rows) {
    ...
    for (const name of [source, target]) if (!nodeSet.has(name)) { nodeSet.add(name); nodeNames.push(name); }
    const key = `${source}\u0000${target}`; linkTotals.set(key, (linkTotals.get(key) ?? 0) + value);
  }
...
export function buildChartModel(block, dataset) {
  ...
  if (block.interaction?.zoom && !["cartesian", "scatter", "heatmap"].includes(model.kind)) throw new Error(`Chart ${block.id ?? "(unnamed)"} requests zoom for unsupported ${model.kind} chart`);
  if (block.interaction?.zoom && !model.options.dataZoom) model.options.dataZoom = [{ type: "inside" }, { type: "slider", bottom: 12 }];
