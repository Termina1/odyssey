...
declare global {
	interface Window {
		echarts: { getInstanceByDom: (node: Element) => RuntimeChart };
	}
}
type Layout = {
	overflow: number;
	blocks: Array<{ left: number; right: number; width: number; top: number; bottom: number }>;
	duplicateIds: string[];
	emptyAlt: number;
...
type Result = {
	htmlPath: string;
	screenshots: string[];
	checks: Array<{ viewport: string; chartCount: number; overflow: number; duplicateIds: number }>;
};
...
try {
	for (const target of targets) {
		...
		assert.equal(
			await page.locator('.re-chart[role="img"][aria-label]').count(),
			chartCount,
			`${target.name}: chart accessibility labels`,
		);
		const layout: Layout = await page.evaluate(() => {
			const viewport = window.innerWidth;
			const overflow = document.documentElement.scrollWidth - viewport;
			const blocks = [...document.querySelectorAll(".re-block")].map((node) => {
			...
			return {
				...
				blocks,
				duplicateIds,
				emptyAlt: [...document.images].filter((image) => !image.alt).length,
				chartSummaries: [...document.querySelectorAll("[data-chart-summary]")].filter(
					(node) => !node.textContent?.trim(),
				).length,
			};
		});
		assert.ok(layout.overflow <= 1, `${target.name}: horizontal document overflow ${layout.overflow}px`);
		assert.equal(layout.duplicateIds.length, 0);
		assert.equal(layout.emptyAlt, 0);
		assert.equal(layout.chartSummaries, 0);
		assert.ok(layout.blocks.every((block) => block.width > 0 && block.left >= -1 && block.right <= target.width + 1));
		const sparseChart = page.locator(".re-block-chart").filter({ hasText: "Отгрузки ESL-модулей" }).first();
		assert.equal(await sparseChart.count(), 1);
		assert.equal(await sparseChart.locator(".re-legend-controls").count(), 1);
		assert.doesNotMatch(await sparseChart.locator("[data-chart-summary]").innerText(), /:\s*0(?:\s|$)/);
		const sparseOption = await sparseChart
			.locator(".re-chart")
			.evaluate((node): { legendVisible: boolean | undefined; values: Array<Array<number | null>> } => {
				const option = window.echarts.getInstanceByDom(node).getOption();
				return {
					legendVisible: option.legend?.some((legend) => legend.show),
					values: option.series.map((series) =>
						series.data.map((datum) => (typeof datum?.value === "number" ? datum.value : null)),
					),
				};
			});
		assert.equal(sparseOption.legendVisible, false);
		assert.deepEqual(sparseOption.values, [
			[370, null],
			[null, 500],
		]);
		...
		assert.equal(await revenueChart.locator(".re-legend-controls").count(), 0);
		const revenueOption = await revenueChart.locator(".re-chart").evaluate((node) => {
			const option = window.echarts.getInstanceByDom(node).getOption();
			return option.series.map((series) => ({
				type: series.type,
				values: series.data.map((datum) => (typeof datum?.value === "number" ? datum.value : null)),
			}));
		});
		...
		assert.equal(await ownershipChart.count(), 1);
		assert.equal(await ownershipChart.locator(".re-legend-controls").count(), 0);
		assert.doesNotMatch(await ownershipChart.innerText(), /annotation-only|^bar$/m);
		assert.match(await ownershipChart.innerText(), /Несопоставимый ориентир/);
		...
		assert.equal(
			ownershipOption.categories.some((category) => String(category).includes("2023")),
			false,
		);
		...
		if (target.name === "desktop") {
			...
			assert.equal(await netronix.count(), 1);
			const filter = netronix.locator(".re-series-toggle").first();
			assert.equal(await filter.getAttribute("aria-pressed"), "true");
			await filter.click();
			assert.equal(await filter.getAttribute("aria-pressed"), "false");
		}
		const screenshot = resolve(outputDir, `${target.name}.png`);
		await page.screenshot({ path: screenshot, fullPage: true });
		result.screenshots.push(relative(cwd, screenshot));
		result.checks.push({
			viewport: target.name,
			chartCount,
			overflow: layout.overflow,
			duplicateIds: layout.duplicateIds.length,
		});
		await page.close();
	}
} finally {
