...
import { z } from "zod";
import { SectionWorkItems, ScreenshotManifest } from "../contracts/index.js";
...
const writeJson = async (path: string, value: unknown): Promise<void> => { const full = resolve(work, path); await mkdir(dirname(full), { recursive: true }); await writeFile(full, `${JSON.stringify(value, null, 2)}\n`); };
const run = (script: string, env: Record<string, string>): string => execFileSync("tsx", [resolve(root, script)], { cwd: work, env: { ...process.env, ...env }, encoding: "utf8" }).trim();
...
if (!run("scripts/validate-experience.ts", { PLAN_FILE: "artifacts/plan.json", EVIDENCE_FILE: "artifacts/evidence.json", EXPERIENCE_FILE: "artifacts/experience.json" }).includes("EXPERIENCE_VALID")) throw new Error("experience validation failed");
if (!run("scripts/validate-visual-input.ts", { REQUEST_JSON: JSON.stringify(request), INPUT_FILE: "artifacts/visual.json", EVIDENCE_FILE: "artifacts/evidence.json" }).includes("VISUAL_INPUT_VALID")) throw new Error("visual validation failed");
const badRequest = { ...request, id: "wrong" };
if (!run("scripts/validate-visual-input.ts", { REQUEST_JSON: JSON.stringify(badRequest), INPUT_FILE: "artifacts/visual.json", EVIDENCE_FILE: "artifacts/evidence.json" }).includes("VISUAL_INPUT_INVALID")) throw new Error("invalid visual id was accepted");
run("scripts/prepare-chapter-work.ts", { PLAN_FILE: "artifacts/plan.json", EXPERIENCE_FILE: "artifacts/experience.json", OUTPUT_PATH: "artifacts/work.json" });
const workItems = await parseJsonFile(resolve(work, "artifacts/work.json"), SectionWorkItems); const item = workItems.items.s1; if (!item) throw new Error("chapter work item missing");
if (!run("scripts/route-chapter-start.ts", { WORK_JSON: JSON.stringify(item) }).includes("START_LAYOUT")) throw new Error("initial chapter route failed");
await writeJson(item.chapterPlanPath, chapterPlan);
run("scripts/assemble-chapter-visual-inputs.ts", { SECTION_ID: "s1", VISUAL_FILES: JSON.stringify(["artifacts/visual.json"]), OUTPUT_PATH: item.visualCatalogPath });
...
if (!run("scripts/validate-elements.ts", { WORK_JSON: JSON.stringify(item), ELEMENTS_FILE: item.elementPath, VISUAL_CATALOG_FILE: item.visualCatalogPath }).includes("ELEMENTS_VALID")) throw new Error("element validation failed");
...
if (!run("scripts/validate-elements.ts", { WORK_JSON: JSON.stringify(item), ELEMENTS_FILE: "artifacts/metric-elements.json", VISUAL_CATALOG_FILE: "artifacts/metric-catalog.json" }).includes("ELEMENTS_VALID")) throw new Error("metric selector validation failed");
...
if (!run("scripts/validate-elements.ts", { WORK_JSON: JSON.stringify(item), ELEMENTS_FILE: "artifacts/metric-elements-invalid.json", VISUAL_CATALOG_FILE: "artifacts/metric-catalog.json" }).includes("ELEMENTS_INVALID")) throw new Error("invalid metric selector was accepted");
if (!run("scripts/validate-chapter.ts", { WORK_JSON: JSON.stringify(item), ELEMENTS_FILE: item.elementPath, CHAPTER_FILE: item.chapterPath }).includes("CHAPTER_VALID")) throw new Error("chapter validation failed");
const rewrite = { reason: "Tighten the opening", chapters: { s1: { owner: "copy", instructions: ["Rewrite the opening"] } } };
run("scripts/route-chapter-rework.ts", { WORK_FILE: "artifacts/work.json", FEEDBACK_JSON: JSON.stringify(rewrite), OUTPUT_PATH: "artifacts/rework.json" });
const reworkItems = await parseJsonFile(resolve(work, "artifacts/rework.json"), SectionWorkItems); const rework = reworkItems.items.s1; if (!rework) throw new Error("rework item missing");
if (!run("scripts/route-chapter-start.ts", { WORK_JSON: JSON.stringify(rework) }).includes("START_COPY")) throw new Error("targeted copy rework route failed");
const layoutRework = { ...item, reworkFeedback: { owner: "layout", reason: "Visual mismatch", instructions: ["Replan visuals"] } };
if (!run("scripts/route-chapter-start.ts", { WORK_JSON: JSON.stringify(layoutRework) }).includes("START_LAYOUT")) throw new Error("targeted layout rework route failed");
run("scripts/assemble-report-document.ts", { PLAN_FILE: "artifacts/plan.json", EVIDENCE_FILE: "artifacts/evidence.json", EXPERIENCE_FILE: "artifacts/experience.json", WORK_FILE: "artifacts/work.json", VISUAL_FILES: JSON.stringify(["artifacts/visual.json"]), OUTPUT_PATH: "artifacts/report-document.json" });
run("engine/render-report.ts", { DOCUMENT_FILE: "artifacts/report-document.json", OUTPUT_PATH: "artifacts/report.html" });
if (!run("scripts/validate-render.ts", { DOCUMENT_FILE: "artifacts/report-document.json", HTML_FILE: "artifacts/report.html", OUTPUT_PATH: "artifacts/render-review.json" }).includes("RENDER_VALIDATED")) throw new Error("render validation failed");
const screenshots = parseJsonText(run("scripts/screenshot-report.ts", { HTML_FILE: "artifacts/report.html", OUTPUT_DIR: "artifacts/screenshots", OUTPUT_PATH: "artifacts/screenshots.json" }), z.object({ output: ScreenshotManifest })).output;
...
if (!run("scripts/visual-rewrite-budget.ts", { QA_VISIT: "1", FEEDBACK_JSON: JSON.stringify(qaFeedback) }).includes("ALLOW_REWRITE")) throw new Error("first visual rewrite was not allowed");
if (!run("scripts/visual-rewrite-budget.ts", { QA_VISIT: "2", FEEDBACK_JSON: JSON.stringify(qaFeedback) }).includes("QA_LIMIT_REACHED")) throw new Error("visual rewrite limit failed");
run("scripts/finalize-visual-warnings.ts", { FEEDBACK_JSON: JSON.stringify(qaFeedback), OUTPUT_PATH: "artifacts/visual-warnings.json" });
