...
Plan one atomic Report Engine chapter. Read its work item, the global experience direction, and the evidence index. Decide the chapter-local composition and only the concrete visual requests needed by this chapter. Every request must serve one verified beat, cite existing evidence ids, and include a fallback.
